...
var (
	batchSize   = 1
	model       = "sphereface"
	graph_url   = "https://s3.amazonaws.com/store.carml.org/models/caffe/sphereface/sphereface_deploy.prototxt"
	weights_url = "https://s3.amazonaws.com/store.carml.org/models/caffe/sphereface/sphereface_model.caffemodel"
	synset_url  = "https://s3.amazonaws.com/store.carml.org/models/caffe/sphereface/pairs.txt"
)

// convert go Image to 1-dim array
func cvtImageTo1DArray(src image.Image, mean []float32, scale float32) ([]float32, error) {
	if src == nil {
	...
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			...
			res[y*w+x] = (float32(b>>8) - mean[0]) / scale
			res[w*h+y*w+x] = (float32(g>>8) - mean[1]) / scale
			res[2*w*h+y*w+x] = (float32(r>>8) - mean[2]) / scale
		}
	}
...
func main() {
	...
	graph := filepath.Join(dir, "sphereface_deploy.prototxt")
	weights := filepath.Join(dir, "sphereface_model.caffemodel")
	synset := filepath.Join(dir, "pairs.txt")
	...
	imagePath := filepath.Join(imgDir, "obama.jpg")
	...
	for ii := 0; ii < batchSize; ii++ {
		resized := transform.Resize(img, 112, 96, transform.Linear)
		res, err := cvtImageTo1DArray(resized, []float32{127.5, 127.5, 127.5}, 128)
		if err != nil {
			panic(err)
		}
